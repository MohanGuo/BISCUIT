...
class LabelEvaluation():
    ...
    def __init__(self, dataset, num_train_epochs=100, entanglement=False):
        super().__init__()
    ...
    def test_model(self, pl_module, point, prop = 0.5):
        ...
        for batch in loader:
            ...
            print(f"Shape of latents in a batch: {latents.shape}, {latents[0]}")
        ...
        losses, dists, norm_dists = encoder.calculate_loss_distance(pred_dict, test_exp_labels)

        # Calculate statistics (R^2, pearson, etc.)
        # Why use test_label instead of test_exp_labels?
        avg_norm_dists, r2_matrix = self.log_R2_statistic(encoder, pred_dict, test_labels, norm_dists, point, pl_module=pl_module, prop=prop)
        # self.log_Spearman_statistics(trainer, encoder, pred_dict, test_labels, pl_module=pl_module)
        if is_training:
            pl_module = pl_module.train()
        return r2_matrix, losses


    def _log_heatmap(self, values, tag, title=None, xticks=None, yticks=None, xlabel=None, ylabel=None, pl_module=None, point=None, prop=0.5):
    ...
    def on_validation_epoch_start(self, pl_module, is_test=False, prop = 0.5):
        ...
        r2_matrix, losses = self.test_model(pl_module, "R2_start", prop)
    ...
    def on_validation_epoch_end(self, pl_module, is_test=False, prop=0.5):
        ...
        r2_matrix, losses = self.test_model(pl_module, "R2_end", prop)
...
def EntangledBaseline(model, DataClass, DATA_FOLDER):
    batch_size = 64

    val_dataset = DataClass(
        data_folder=DATA_FOLDER, split='val_indep', single_image=True, return_targets=False, return_latents=True)
    LE = LabelEvaluation(val_dataset, num_train_epochs=100, entanglement=False)
    LE_entangled = LabelEvaluation(val_dataset, num_train_epochs=100, entanglement=True)
    # props = np.arange(0.0001, 0.001, 0.0001)
    # props = np.arange(0.001, 0.01, 0.001)
    # props = np.arange(0.01, 0.1, 0.01)
    # props = np.arange(0.1, 1, 0.1)
    props = np.arange(0.0001, 1, 0.001)
    # props = [0.1, 0.5]
    # props = [0.5]
    r2_diag = []
    r2_sep = []
    ...
    loss_data_en = []
    for prop in props:
        r2_matrix_start, losses_dis = LE.on_validation_epoch_start(model, is_test=False, prop=prop)
        _, losses_en = LE_entangled.on_validation_epoch_start(model, is_test=False, prop=prop)
        loss_data_dis.append(losses_dis)
        loss_data_en.append(losses_en)
        ## Plot losses
        ...
        # plt.ylabel('Loss')
        # plt.title('Losses for Multiple Target Variables')
        # plt.xticks(rotation=90)
        # plt.tight_layout()
        # plt.savefig('/home/mguo/BISCUIT/run/images/losses_all_causal.png')
        # plt.show()
    

    # plot all
    all_keys = list(loss_data_dis[0].keys())
    ...
    for idx, key in enumerate(all_keys):
        props = list(range(len(loss_data_dis)))
        loss_dis = [losses[key] for losses in loss_data_dis]
        loss_en = [losses[key] for losses in loss_data_en]
        ax = axes[idx]
        ax.plot(props, loss_dis, marker='o', label='Disentangled')
        ax.plot(props, loss_en, marker='x', label='Entangled')
        ax.set_title(f'Losses of {key} for Different Props')
        ax.set_xlabel('Prop Index')
        ax.set_ylabel('Losses')
        ax.grid(True)

    plt.tight_layout()
    plt.savefig('/home/mguo/BISCUIT/run/images/all_losses_comparison.png')
    plt.show()

def __main__():
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    CHECKPOINT_FILE_ithor = '/home/mguo/BISCUIT/data/ithor/ithor/models/AE_40l_64hid.ckpt'
    CHECKPOINT_FILE_demo_ithor = '/home/mguo/BISCUIT/pretrained_model/pretrained_models/BISCUITNF_iTHOR/BISCUITNF_40l_64hid.ckpt'
    CHECKPOINT_FILE_AE = '/home/mguo/BISCUIT/pretrained_model/pretrained_models/AE_iTHOR/AE_40l_64hid.ckpt'
    DATA_FOLDER_ithor = '/home/mguo/BISCUIT/data/ithor/ithor'
    # DATA_FOLDER_DEMO = '/home/mguo/BISCUIT/data/ithor/ithor/demo_data/ithor'

    CHECKPOINT_FILE_voronoi = '/home/mguo/BISCUIT/run/checkpoints/BISCUITVAE/BISCUITVAE_12l_6b_32hid_voronoi_6vars_nf_hard_rand_seed42/version_1/checkpoints/epoch=99-step=58500.ckpt'
    # CHECKPOINT_FILE_AE = '/home/mguo/BISCUIT/pretrained_model/pretrained_models/AE_iTHOR/AE_40l_64hid.ckpt'
    DATA_FOLDER_voronoi = '/home/mguo/BISCUIT/data/voronoi/robotics_voronoi_6vars_nf_hard_rand_seed42'

    # iTHOR
    # model = BISCUITNF.load_from_checkpoint(CHECKPOINT_FILE_demo_ithor, autoencoder_checkpoint=CHECKPOINT_FILE_AE)
    # model.to(device)
    # model.eval()
    # DataClass = iTHORDataset
    # DATA_FOLDER = DATA_FOLDER_ithor

    # Voronoi
    model = BISCUITVAE.load_from_checkpoint(CHECKPOINT_FILE_voronoi)
    model.to(device)
    model.eval()
    DataClass = VoronoiDataset
    DATA_FOLDER = DATA_FOLDER_voronoi

    EntangledBaseline(model, DataClass, DATA_FOLDER)
